...
- Prefer the standard library and existing internal libraries before adding new
  third-party packages.
- Add new dependencies only when they provide clear, sustained value that cannot
  be reasonably implemented or maintained in-house.
- Minimize transitive dependency chains and keep the dependency graph shallow.
- Document the rationale for each new dependency and remove unused packages
  promptly.
