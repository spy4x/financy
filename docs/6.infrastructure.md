...
1. **Flexibility and Modularity**:
   ...
   - Scalable architecture to handle traffic and data increases by scaling
     specific portions of the stack (e.g., backend API or Redis). -server
...
- **Horizontal Scalability**:
  - Stateless architecture means additional containers can be spun up to handle
    API load spikes.
  - Redis used for shared data caching across multiple API instances.
- **WebSocket or Server-Sent Events (Optional Future Feature)**:
  - Redis Pub/Sub provides the foundation for adding real-time notifications of
