...
async function load(): Promise<void> {
  ...
  try {
    ...
    const result = data.map(userPushTokenSchema.parse)
    isSubscribed = result.find((token: UserPushToken) => token.deviceId === deviceId)
    set(store, {
      list: result,
      listOp: { inProgress: false, error: null, result: result },
    })
    if (isSubscribed) {
      set(store, {
        isSubscribed: true,
...
export const pushNotification = {
  ...
  subscribe: async () => {
    ...
    try {
      ...
      set(store, {
        subscribeOp: { inProgress: false, error: error.message, result: null },
      })
      console.error(error)
      toast.error({ title: "Failed to subscribe", body: error.message })
    }
  },
  unsubscribe: async () => {
    const deviceId = store.value.deviceId
    ...
    try {
      ...
      set(store, {
        unsubscribeOp: { inProgress: false, error: error.message, result: null },
      })
      console.error(error)
      toast.error({ title: "Failed to unsubscribe", body: error.message })
    }
  },
  unsubscribeDevice: async (deviceId: string) => {
    ...
    try {
      ...
      set(store, {
        unsubscribeDeviceOps: {
          ...store.value.unsubscribeDeviceOps,
          [deviceId]: { inProgress: false, error: error.message, result: null },
        },
      })
