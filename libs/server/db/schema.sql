...
-- migrations, users, currencies, tags, exchange_rates → exchange_rate_api_usage, 
-- exchange_rate_overrides, exchange_rate_alerts, user_keys, user_push_tokens, groups
...
COMMENT ON TABLE exchange_rate_api_usage IS 'Tracks API usage for exchange rate providers for monitoring and rate limiting';
COMMENT ON COLUMN exchange_rate_api_usage.provider IS 'Exchange rate provider name (e.g., exchangerate-api.com, fixer.io)';
COMMENT ON COLUMN exchange_rate_api_usage.response_time_ms IS 'API response time in milliseconds';
...
COMMENT ON TABLE exchange_rate_overrides IS 'Manual exchange rate overrides set by admins for emergency situations or regulatory compliance';
COMMENT ON COLUMN exchange_rate_overrides.reason IS 'Required reason for manual override (audit trail)';
...
COMMENT ON COLUMN exchange_rate_overrides.overridden_by IS 'User ID of admin who set the override';
...
COMMENT ON TABLE exchange_rate_alerts IS 'Tracks significant exchange rate changes for notifications and monitoring';
COMMENT ON COLUMN exchange_rate_alerts.alert_type IS '1=minor_change (>2%), 2=significant_change (>5%), 3=major_change (>10%)';
COMMENT ON COLUMN exchange_rate_alerts.notified_at IS 'Timestamp when notification was sent (NULL if not yet sent)';
